...
1. **없는 사실을 쓰지 마라.** 지원자 이력은 아래 준 것만, 사무소 프로젝트는 아래 준 목록만 쓴다.
   프로젝트 목록이 비어 있으면 **프로젝트 대신 그 사무소의 분야·공고에 적힌 업무**를 근거로
   관심 이유를 쓴다. 없는 프로젝트를 지어내면 그 메일은 보내는 순간 신뢰를 잃는다.
2. 과장하지 마라. 경력 0년이면 0년인 채로 쓴다. "열정" 같은 상투어 대신 구체적인 것으로 채운다.
3. **본문은 반드시 이 순서다.**
   ① 인사 + 한 줄 자기소개
   ② **그 사무소의 구체적인 프로젝트를 이름으로 언급하고, 왜 관심 있게 봤는지**
      — 지원자의 이력(전시·뮤지엄 공간 설계, 환경분석, 모형·시각화)과 **연결해서** 쓴다.
      "귀사의 OO 프로젝트에서 △△가 인상적이었고, 제가 □□에서 한 작업과 닿아 있다" 식.
      막연히 "귀사에 관심이 있습니다" 라고 쓰지 마라. 그건 아무 회사에나 보내는 메일이다.
   ③ 본인이 무엇을 할 수 있는지 (툴·경험)
   ④ 확인하고 싶은 것 (외국인 지원 가능 여부·비자·어학)
   ⑤ 첨부 안내 + 맺음말
   길이는 400~550자. ②가 이 메일의 핵심이므로 여기에 가장 많이 쓴다.
...
위 정보만 가지고 문의 메일을 써라.
프로젝트 목록이 있으면 **반드시 그중 1~2개를 이름으로 언급하고**, 지원자 이력과 연결해라.
목록이 비어 있으면 공고에 적힌 업무 내용을 근거로 쓴다. 지어내지 마라.
