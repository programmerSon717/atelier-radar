...
3. **회사명(company)은 그 언어권에서 통용되는 표기로 적는다.** 영어면 로마자 표기
   ...
   **프로젝트명·건물명·역명 같은 고유명사(firm_projects 포함)는 원문을 먼저 쓰고,
   그 언어권에서 실제로 통용되는 공식 표기를 괄호에 덧붙인다.**
     · 東京スカイツリー → zh_TW "東京スカイツリー (東京晴空塔)" / en "東京スカイツリー (Tokyo Skytree)"
     · 東京湾アクアライン → zh_TW "東京湾アクアライン (東京灣跨海公路)"
     · 品川新駅（高輪ゲートウェイ駅） → zh_TW "品川新駅（高輪ゲートウェイ駅） (品川新站·高輪Gateway站)"
   원문을 지우지 마라 — 그 이름으로 검색해야 하는 사람이 읽는다.
   **통용 표기를 확실히 모르면 괄호를 아예 붙이지 마라.** 음차해서 지어내는 것이
   안 붙이는 것보다 나쁘다. 한자가 그 언어에서도 그대로 쓰이면(阪神甲子園球場 등)
   괄호가 필요 없다 — 굳이 같은 말을 두 번 쓰지 마라.
