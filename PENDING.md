...
## 다음에 할 것
0. **작품명 병기 순서를 정할 것** — 일본 건은 `東京スカイツリー (東京晴空塔)` 로
   원문이 먼저인데, 한국 건은 `首爾大學醫學圖書館 (서울대학교 의학도서관)` 처럼
   뒤집혀 나온다. `안녕하우스` 처럼 병기가 아예 안 붙는 것도 있다.
   대만분 기준으로는 중문이 먼저인 게 나을 수도 있어 **사용자 판단이 필요하다**
...
## 끝난 것 (2026-09-09)
- 텔레그램을 웹 기준으로 재작성 — 번체중문, 웹과 같은 항목·순서. 문구 원본은
  `config/locales/*.yaml` 하나이고 `site/ui.js` 는 거기서 생성된다
- 토픽 정리 — `✅可以應徵`(419) 신설, 나라 토픽 이름을 중문으로 (韓國11·日本12·台灣13).
  id 를 `settings.yaml` 에 적어 뒀으므로 시크릿을 안 넣어도 돈다
- 번역을 발송 **전에** 하도록 옮김. 조각 보수 우선 + 3병렬로 속도 3배
